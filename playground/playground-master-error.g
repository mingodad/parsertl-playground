...
start : file ;
file : directives "%%" grules "%%" rx_directives rx_macros "%%" rx_rules "%%" erules_opt ;
...
// Error rules
erules_opt : %empty | erules "%%" ;
erules : %empty | erules erule ;
erule : Literal ':' production ';' ;


%%

%x OPTION GRULE MACRO REGEX RULE ID RXDIRECTIVES PRODALIAS ERULE
...
<GRULE,ERULE>";"	';'
<GRULE,ERULE>[ \t]+|{NL}	skip()
<GRULE,ERULE>"%empty"	"%empty"
<GRULE,ERULE>"%%"<MACRO>	"%%"
<INITIAL,GRULE,ERULE>{c_comment}	skip()
/* Bison supports single line comments */
<INITIAL,GRULE,ERULE>"//".*	skip()
<INITIAL,GRULE,ERULE,ID>'({literal_common}|[^'\\])'|["]({literal_common}|[^"\\])+["]	Literal
<INITIAL,GRULE,ERULE,ID,RXDIRECTIVES>[.A-Z_a-z][-.0-9A-Z_a-z]*	Name
<ID>[1-9][0-9]*	Number
