...
namespace lexertl
{
    ...
    public:
        ...
        explicit basic_memory_file(const char_type* data, size_t size):
            _data(data), _size(size), _isDataBorrowed(true)
        {
        }
        ...
        void open(const char_type* data, size_t data_size)
        {
            if (_data)
            {
                close();
